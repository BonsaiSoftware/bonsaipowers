...
4. **Research unknowns** — mandatory for any brainstorm involving external libraries, APIs, cloud services, or framework features; skip ONLY for pure internal refactors or config-only tweaks with zero external dependencies. Two required checks — context7 feasibility spot-checks AND WebSearch prior-art scans — governed by the HARD-GATE in the "Researching unknowns" section below. Fire all queries in parallel (one message, multiple tool calls).
5. **Propose 2-3 approaches** — with trade-offs and your recommendation. Research from step 4 should show up here: approaches that failed a feasibility check are off the table, approaches surfaced by prior-art are on it. The **Azure-native preference** (see "Exploring approaches" below) applies to every approach set.
...
<!-- BONSAI TIER-2 EDIT: Step 1 (structured codebase recon additions), step 4 (research unknowns via context7 + WebSearch with HARD-GATE on BOTH tools), and step 5 (Azure-native preference in approach proposals) are team-specific additions not in upstream. On upstream merge, ensure these steps survive and the numbering stays consistent (upstream has no step 4; its steps 1-9 map to our 1-3, 5-10 with step 1 upgraded). The canonical rule text lives ONCE in the "Researching unknowns" and "Exploring approaches" process sections (HARD-GATE requiring at least one context7 query AND at least one WebSearch query, run in parallel; Azure-native preference) — checklist steps 4-5 are deliberately one-line pointers to those sections, do not re-expand them. See docs/bonsai/tier-2-edits.md. -->
...
**Run the research in parallel:** the queries are independent, so issue every context7 and WebSearch call in a single message (multiple tool calls at once) instead of one at a time — serializing them only adds latency.
